...
- Keep `transfomer-demo` tokenizer integration vendored under `demo/transfomer-demo/third_party/tokenizers-cpp`, wired through `add_subdirectory`, with an opt-out CMake toggle (`TRANSFOMER_DEMO_ENABLE_TOKENIZERS_CPP`) for environments without Rust/Cargo.
