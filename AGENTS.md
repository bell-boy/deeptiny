...
- Keep `transfomer-demo` runtime behavior minimal: simple CLI chat loop that prints raw input/output token IDs plus decoded text, with generation delegated to `Transformer::GenerateAsync(...)` so output token IDs stream as they are produced.
