...
- Keep `transfomer_demo_benchmark` as a forward-only profiling executable in `demo/transfomer-demo`: tokenize fixed `"hello world"` input, run exactly one model forward pass (no sampling/generation), auto-download `model.safetensors` to `model_files/` when missing, and profile via `gperftools` (`libprofiler`/`pprof`) instead of `gprof`; use `TRANSFOMER_DEMO_PROFILE_PATH` for output file overrides (reserve `CPUPROFILE` for gperftools auto-start behavior).
