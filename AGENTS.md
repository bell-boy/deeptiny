...
- Keep `transfomer_demo_benchmark` as a forward-only profiling executable in `demo/transfomer-demo`: tokenize fixed `"hello world"` input, run model forward pass (no sampling/generation), and build with `-pg` for `gprof`.
