...
- Keep profiling entrypoints split as `transfomer_demo_benchmark_prefill` and `transfomer_demo_benchmark_generation` in `demo/transfomer-demo`: instantiate `transfomer_demo::Transformer` directly without safetensors weight loading, tokenize the fixed long-form eval sentence from benchmark sources, auto-download `tokenizer.json` to `model_files/` when missing, and compile both targets with `-pg` for `gprof` profiling.
- Keep `nn::KVCache` append behavior amortized `O(1)` by growing capacity geometrically and writing into preallocated key/value storage instead of reallocating on every token append.
