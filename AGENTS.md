...
- Keep MLP hidden activation typed via `nn::GatedMLP::HiddenAct` with `ReLU` default, and map demo/config activation strings explicitly (`relu` / `silu`) with strict validation.
...
- Keep `transfomer-demo` autoregressive completion centralized on `Transformer::{Generate,GenerateAsync}` rather than duplicating sampling loops in callers like `main.cc`.
...
- Keep `transfomer-demo` default `dev`/`release` presets tokenizer-enabled (`TRANSFOMER_DEMO_ENABLE_TOKENIZERS_CPP=ON`); reserve `OFF` for explicit bench/lightweight presets.
...
- Keep SmolLM2 self-attention projection loader mapping semantic: for `q_proj/k_proj/v_proj`, map HF `[out_dim, in_dim]` weights into deeptiny `[1, heads, in_dim, head_dim]` by unflattening `out_dim` into `(heads, head_dim)` (not by plain last-two transpose alone).
- Keep `transfomer-demo` runtime behavior minimal: simple CLI chat loop that streams decoded output text only (no token ID printing), with generation delegated to `Transformer::GenerateAsync(...)`.
- Keep `transfomer-demo` chat prompt formatting aligned with the SmolLM template (`<|im_start|>...<|im_end|>` blocks), including the default SmolLM system message when the first message is non-system and an assistant generation prompt suffix.
- Keep `transfomer-demo` generation benchmark (`transfomer_generation_benchmark`) weight-file free: construct SmolLM2 via the uninitialized factory, start from one token, and generate 128 tokens for profiling runs.
