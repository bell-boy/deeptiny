...
- Keep `transfomer_demo_benchmark` as a forward-only profiling executable in `demo/transfomer-demo`: tokenize fixed `"hello world"` input, run exactly one model forward pass (no sampling/generation), auto-download `model.safetensors` to `model_files/` when missing, and compile with `-pg` for `gprof` profiling.
