...
- Keep `transfomer_demo_benchmark` as a forward-only profiling executable in `demo/transfomer-demo`: instantiate `transfomer_demo::Transformer` directly without safetensors weight loading, tokenize the fixed long-form eval sentence from `benchmark.cc`, run exactly one model forward pass (no sampling/generation), auto-download `tokenizer.json` to `model_files/` when missing, and compile with `-pg` for `gprof` profiling.
