...
- Keep `transfomer-demo` autoregressive completion centralized on `Transformer::Generate(...)` rather than duplicating sampling loops in callers like `main.cc`.
...
- Keep `transfomer-demo` runtime behavior minimal: simple CLI chat loop that prints raw input/output token IDs plus decoded text, with generation delegated to `Transformer::Generate(...)`.
