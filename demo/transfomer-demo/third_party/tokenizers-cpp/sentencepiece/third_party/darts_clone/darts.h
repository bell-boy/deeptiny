...
#define DARTS_THROW(msg) \
  throw Darts::Details::Exception(__FILE__ ":" DARTS_LINE_STR ": exception: " msg)
...
class DoubleArrayImpl {
 public:
  ...
  int open(const char* file_name, const char* mode = "rb", std::size_t offset = 0,
           std::size_t size = 0);
...
int DoubleArrayImpl<A, B, T, C>::open(const char* file_name, const char* mode, std::size_t offset,
                                      std::size_t size) {
#ifdef _MSC_VER
  std::FILE* file;
  ...
  if (units[0].label() != '\0' || units[0].has_leaf() || units[0].offset() == 0 ||
      units[0].offset() >= 512) {
    std::fclose(file);
    return -1;
  }
    ...
    for (std::size_t i = 0; i < length; ++i) {
      ...
      if (unit.has_leaf()) {
        if (num_results < max_num_results) {
          set_result(&results[num_results], static_cast<value_type>(array_[node_pos].value()),
                     i + 1);
        }
        ++num_results;
      }
...
template <typename A, typename B, typename T, typename C>
inline typename DoubleArrayImpl<A, B, T, C>::value_type DoubleArrayImpl<A, B, T, C>::traverse(
    const key_type* key, std::size_t& node_pos, std::size_t& key_pos, std::size_t length) const {
...
class AutoPool {
 public:
  ...
  void append() {
    if (size_ == capacity_) resize_buf(size_ + 1);
    new (&(*this)[size_++]) T;
  }
 ...
 public:
  ...
  id_type rank(std::size_t id) const {
    ...
    return ranks_[unit_id] +
           pop_count(units_[unit_id] & (~0U >> (UNIT_SIZE - (id % UNIT_SIZE) - 1)));
  }
...
class Keyset {
 public:
  Keyset(std::size_t num_keys, const char_type* const* keys, const std::size_t* lengths,
         const T* values)
  ...
  uchar_type keys(std::size_t key_id, std::size_t char_id) const {
    if (has_lengths() && char_id >= lengths_[key_id]) return '\0';
...
class DoubleArrayBuilder {
 ...
 private:
  ...
  template <typename T>
  void build_from_keyset(const Keyset<T>& keyset, std::size_t begin, std::size_t end,
                         std::size_t depth, id_type dic_id);
  template <typename T>
  id_type arrange_from_keyset(const Keyset<T>& keyset, std::size_t begin, std::size_t end,
                              std::size_t depth, id_type dic_id);
...
inline void DoubleArrayBuilder::build_from_dawg(const DawgBuilder& dawg, id_type dawg_id,
                                                id_type dic_id) {
...
inline id_type DoubleArrayBuilder::arrange_from_dawg(const DawgBuilder& dawg, id_type dawg_id,
                                                     id_type dic_id) {
  labels_.resize(0);
...
template <typename T>
void DoubleArrayBuilder::build_from_keyset(const Keyset<T>& keyset, std::size_t begin,
...
template <typename T>
id_type DoubleArrayBuilder::arrange_from_keyset(const Keyset<T>& keyset, std::size_t begin,
...
template <typename A, typename B, typename T, typename C>
int DoubleArrayImpl<A, B, T, C>::build(std::size_t num_keys, const key_type* const* keys,
