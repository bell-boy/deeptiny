  ...
  SentenceSelector(TrainerInterface::Sentences* sentences, const TrainerSpec& spec)
    ...
    if (spec_->input_sentence_size() > 0) {
      if (spec_->shuffle_input_sentence()) {
        ...
        sampler_ = std::make_unique<Sampler>(sentences, spec_->input_sentence_size(), kSeed);
      } else {
...
MultiFileSentenceIterator::MultiFileSentenceIterator(const std::vector<std::string>& files)
    : files_(files) {
  Next();
}
...
bool TrainerInterface::IsValidSentencePiece(const string_util::UnicodeText& sentencepiece) const {
  ...
  constexpr unicode_script::ScriptType kAnyType =
      static_cast<unicode_script::ScriptType>(std::numeric_limits<char32>::max());
  ...
  for (size_t pos = 0; pos < sentencepiece.size(); ++pos) {
    ...
    if (c == kWSChar) {
      ...
      if (!trainer_spec_.allow_whitespace_only_pieces() || !all_whitespace_piece) {
        if (trainer_spec_.treat_whitespace_as_suffix()) {
          if ((trainer_spec_.split_by_whitespace() && pos < sentencepiece.size() - 1) ||
              (!trainer_spec_.split_by_whitespace() && pos < sentencepiece.size() - 1 &&
               pos == 0)) {
            return false;
          }
        } else {
      ...
      if (trainer_spec_.split_by_unicode_script() && s != kAnyType && prev_script != kAnyType &&
          prev_script != s) {
        return false;
      }
...
util::Status TrainerInterface::LoadSentences() {
  ...
  CHECK_OR_RETURN(trainer_spec_.input_format().empty() || trainer_spec_.input_format() == "text" ||
                  trainer_spec_.input_format() == "tsv")
      << "Supported formats are 'text' and 'tsv'.";

  CHECK_OR_RETURN((sentence_iterator_ != nullptr && trainer_spec_.input().empty()) ||
                  (sentence_iterator_ == nullptr && !trainer_spec_.input().empty()))
      << "SentenceIterator and trainer_spec.input() must be exclusive.";

  CHECK_OR_RETURN((output_model_proto_ != nullptr && trainer_spec_.model_prefix().empty()) ||
  ...
  if (sentence_iterator_ == nullptr) {
    ...
    sentence_iterator_impl = std::make_unique<MultiFileSentenceIterator>(
        std::vector<std::string>(trainer_spec_.input().begin(), trainer_spec_.input().end()));
...
END:
  ...
  if (sentences_.size() == selector.total_size()) {
    ...
    LOG(INFO) << "Sampled " << sentences_.size() << " sentences from " << selector.total_size()
              << " sentences.";
  }

  if (too_long_lines > 0) LOG(INFO) << "Skipped " << too_long_lines << " too long sentences.";
  ...
  if (trainer_spec_.enable_differential_privacy()) {
    ...
    const auto num_workers = std::min<uint64_t>(trainer_spec_.num_threads(), sentences_.size() - 1);
      ...
      for (int n = 0; n < num_workers; ++n) {
        pool->Schedule([&, n]() {
          ...
          for (size_t i = n; i < sentences_.size(); i += num_workers) {
            AddDPNoise<int64_t>(trainer_spec_, generator, &(sentences_[i].second));
          }
        });
      }
    }
  ...
  for (const char32 c : string_util::UTF8ToUnicodeText(trainer_spec_.required_chars())) {
  ...
  for (const auto& w : sentences_) {
    for (const char32 c : string_util::UTF8ToUnicodeText(w.first)) {
      ...
      if (c == 0x0000) {
        LOG(INFO) << "Found null character. The corpus must be encoded in utf-8.";
        continue;
      }
      if (c == 0x0020) {
  ...
  for (const auto& w : Sorted(chars_count)) {
    ...
    if (!trainer_spec_.use_all_vocab() && coverage >= trainer_spec_.character_coverage()) {
      LOG(INFO) << "Done: " << 100.0 * coverage << "% characters are covered.";
      break;
    }
    accumulated_chars_count += w.second.second;
    CHECK_NE_OR_RETURN(w.first, 0x0020) << "space must not be included in normalized string.";
  ...
  LOG(INFO) << "Final character coverage=" << 1.0 * accumulated_chars_count / all_chars_count;
  ...
  if (trainer_spec_.model_type() != TrainerSpec::WORD &&
    ...
    CHECK_LE_OR_RETURN(static_cast<int>(required_chars_.size() + meta_pieces_.size()),
                       trainer_spec_.vocab_size())
        << "Vocabulary size is smaller than required_chars. " << trainer_spec_.vocab_size()
        << " vs " << required_chars_.size() + meta_pieces_.size() << ". "
...
void TrainerInterface::SplitSentencesByWhitespace() {
  ...
  for (const auto& s : sentences_) {
    for (const auto& w : SplitIntoWords(s.first, trainer_spec_.treat_whitespace_as_suffix(),
...
#define CHECK_PIECE(piece)                                  \
  ...
  *(model_proto->mutable_normalizer_spec()) = normalizer_spec_;
  ...
  if (!trainer_spec_.hard_vocab_limit() || trainer_spec_.model_type() == TrainerSpec::CHAR) {
    CHECK_GE_OR_RETURN(trainer_spec_.vocab_size(), model_proto->pieces_size());
    CHECK_GE_OR_RETURN(trainer_spec_.vocab_size(), static_cast<int32_t>(dup.size()));
    model_proto->mutable_trainer_spec()->set_vocab_size(model_proto->pieces_size());
  } else {
    CHECK_EQ_OR_RETURN(trainer_spec_.vocab_size(), model_proto->pieces_size())
        << absl::StrFormat("Vocabulary size too high (%d). Please set it to a value <= %d.",
                           trainer_spec_.vocab_size(), model_proto->pieces_size());
    CHECK_EQ_OR_RETURN(trainer_spec_.vocab_size(), static_cast<int32_t>(dup.size()));
  }
  ...
  for (const auto& piece : model_proto.pieces()) {
    if (piece.piece().find_first_of(" \t\r\n") != std::string::npos) {
      LOG(WARNING) << "The piece [" << piece.piece()
                   << "] contains escaped characters that break the format of " << filename;
    }
  }
...
util::Status TrainerInterface::InitMetaPieces() {
  ...
  auto insert_id = [&has_unk, this](int id, const std::string& w) -> bool {
    if (id < 0) return true;
    if (id >= trainer_spec_.vocab_size() || meta_pieces_.find(id) != meta_pieces_.end() ||
        (has_unk && w == trainer_spec_.unk_piece()))
      return false;
    if (w == trainer_spec_.unk_piece()) has_unk = true;
    meta_pieces_[id] =
        std::make_pair(w, w == trainer_spec_.unk_piece() ? ModelProto::SentencePiece::UNKNOWN
  ...
  auto insert_meta_symbol = [&id, &dup, this](
    ...
    if (!dup.insert(w).second) {
      return util::InternalError(
          absl::StrCat(w, " is already defined. duplicated symbols are not allowed."));
    }

    if (w == trainer_spec_.unk_piece()) {
      return util::InternalError(absl::StrCat(trainer_spec_.unk_piece(),
  ...
  for (const auto& w : trainer_spec_.user_defined_symbols()) {
    RETURN_IF_ERROR(insert_meta_symbol(w, ModelProto::SentencePiece::USER_DEFINED));
  }

  if (trainer_spec_.byte_fallback()) {
    for (int i = 0; i < 256; ++i) {
      RETURN_IF_ERROR(insert_meta_symbol(ByteToPiece(i), ModelProto::SentencePiece::BYTE));
    }
  }
