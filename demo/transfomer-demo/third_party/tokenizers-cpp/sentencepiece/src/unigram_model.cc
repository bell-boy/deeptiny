...
Lattice::Node* Lattice::Insert(int pos, int length) {
  ...
  node->pos = pos;
  node->length = length;
  const int utf8_length = static_cast<int>(surface(pos + length) - surface(pos));
...
Lattice::LatticePathWithScore Lattice::Viterbi() {
  ...
  for (Node* node = begin_nodes_[len][0]->prev; node->prev != nullptr; node = node->prev) {
...
std::vector<float> Lattice::ForwardAlgorithm(float inv_theta) const {
  ...
  for (int pos = 0; pos <= len; ++pos) {
    for (Node* rnode : begin_nodes_[pos]) {
      for (Node* lnode : end_nodes_[pos]) {
        alpha[rnode->node_id] =
            LogSumExp(alpha[rnode->node_id], inv_theta * lnode->score + alpha[lnode->node_id],
...
std::vector<float> Lattice::BackwardAlgorithm(float inv_theta) const {
  ...
  for (int pos = len; pos >= 0; --pos) {
    for (Node* lnode : end_nodes_[pos]) {
      for (Node* rnode : begin_nodes_[pos]) {
        beta[lnode->node_id] = LogSumExp(beta[lnode->node_id], rnode->score + beta[rnode->node_id],
  ...
  for (int pos = 0; pos < len; ++pos) {
    for (Node* node : begin_nodes_[pos]) {
      if (node->id >= 0) {
        ...
        (*expected)[node->id] +=
            freq * std::exp(static_cast<double>(alpha[node->node_id] + node->score +
                                                beta[node->node_id] - Z));
      }
    }
  }
    ...
    for (Node* rnode : begin_nodes_[pos]) {
      for (Node* lnode : end_nodes_[pos]) {
        ...
        const float lnode_transition_prob =
            ((inv_theta * lnode->score) + alpha[lnode->node_id] - alpha[rnode->node_id]);
        H[rnode->node_id] +=
            std::exp(lnode_transition_prob) * (H[lnode->node_id] + lnode_transition_prob);
      }
    }
  }
...
Hypothesis* CloneHypAndDependents(const Hypothesis* to_clone,
...
std::vector<Lattice::LatticePathWithScore> Lattice::NBest(size_t nbest_size, bool sample,
                                                          float inv_theta) {
  if (nbest_size < 1) {
  ...
  class HypothesisComparator {
   public:
    const bool operator()(Hypothesis* h1, Hypothesis* h2) { return (h1->fx < h2->fx); }
  };

  using Agenda = std::priority_queue<Hypothesis*, std::vector<Hypothesis*>, HypothesisComparator>;
  ...
  while (!agenda.empty()) {
    ...
    if (sample) {
      ...
      for (int i = 0; i < end_nodes(node->pos).size(); i++) {
        ...
        probs[i] = top->gx + alpha[lnode->node_id] + (inv_theta * lnode->score) - Z;
      ...
      for (int i = 0; i < adjusted_probs.size(); i++) {
        ...
        const float v =
            top->fx - perturbed_probs[i] + std::log1p(-std::exp(perturbed_probs[i] - max_score));
        adjusted_probs[i] =
            top->fx - std::max(static_cast<float>(0.0), v) - std::log1p(std::exp(-std::abs(v)));
      }
    }
    ...
    for (int i = 0; i < end_nodes(node->pos).size(); i++) {
      ...
      if (sample) {
        ...
        hyp->fx = lnode->backtrace_score + top->gx;  // backtrace_score is h(node).
      }
      hyp->next = top;
      agenda.push(hyp);
    }
    ...
    if (hypothesis_allocator.size() >= kOneBillion) {
      if (!printed_memory_warning) {
        ...
        LOG(WARNING) << "Allocator size exceeds " << kOneBillion << " with an example of length "
                     << this->size();
      }
    }
    ...
    if (agenda.size() >= kMaxAgendaSize) {
      ...
      shrink_count++;
      LOG(WARNING) << "Too big agenda size " << agenda.size() << ". Shrinking (round "
                   << shrink_count << ") down to " << size << ".";
      for (int i = 0; i < size; ++i) {
        const Hypothesis* top_hyp = agenda.top();
        Hypothesis* cloned_hyp = CloneHypAndDependents(top_hyp, &clone_map, &new_allocator);
...
std::vector<Lattice::Node*> Lattice::Sample(float inv_theta) {
  ...
  while (true) {
    ...
    for (const Node* lnode : end_nodes_[node->pos]) {
      probs.push_back(
          std::exp(static_cast<double>(alpha[lnode->node_id] + inv_theta * lnode->score - Z)));
    }
...
void Model::PopulateNodes(Lattice* lattice) const {
  ...
  // +1 just in case.
  std::vector<Darts::DoubleArray::result_pair_type> trie_results(trie_results_size_ + 1);
  ...
  for (int begin_pos = 0; begin_pos < len; ++begin_pos) {
    ...
    const size_t num_nodes = trie_->commonPrefixSearch(
        begin, trie_results.data(), trie_results.size(), static_cast<int>(end - begin));
    ...
    for (size_t k = 0; k < num_nodes; ++k) {
      const int length = get_chars_length(begin_pos, begin + trie_results[k].length);
      ...
      node->score = IsUserDefinedInlined(id) ? (length * max_score_ - 0.1) : GetScoreInlined(id);
...
void Model::BuildTrie(std::vector<std::pair<absl::string_view, int>>* pieces) {
  ...
  for (const auto& p : *pieces) {
    const int num_nodes =
        trie_->commonPrefixSearch(p.first.data(), results.data(), results.size(), p.first.size());
    trie_results_size_ = std::max(trie_results_size_, num_nodes);
  }

  pieces_.clear();

  if (trie_results_size_ == 0) status_ = util::InternalError("no entry is found in the trie.");
}
...
NBestEncodeResult Model::SampleEncodeAndScore(absl::string_view normalized, float inv_theta,
                                              int samples, bool wor, bool include_best) const {
  if (!status().ok() || normalized.empty()) {
    return {};
  }
  ...
  if (wor) {
    ...
    if (include_best) {
      std::vector<std::vector<Lattice::Node*>> nbest_paths(nbest_samples.size());
    ...
    for (auto& it : results) {
      ...
      if (it.second != 0.0) {
        ...
        if (x <= -10) {
          ...
          inclusion_prob = x - (y / 2) + (std::pow(y, 2) / 24) - std::pow(y, 4) / 2880;
        } else {
...
float Model::CalculateEntropy(absl::string_view normalized, float inv_theta) const {
  Lattice lattice;
  lattice.SetSentence(normalized);
  PopulateNodes(&lattice);
...
bool Model::VerifyOutputsEquivalent(absl::string_view expected, absl::string_view actual) const {
  auto compute_unigram_model_score = [this](std::vector<absl::string_view> output_pieces) {
    float total_score = 0;
    const float unk_score = min_score() - kUnkPenalty;
    for (const auto p : output_pieces) {
      ...
      if (id == unk_id_) {
        ...
        total_score += IsUserDefinedInlined(id) ? (length * max_score_ - 0.1) : GetScoreInlined(id);
      }
    }
    return total_score;
  };
  const auto expected_score = compute_unigram_model_score(absl::StrSplit(expected, ' '));
  const auto actual_score = compute_unigram_model_score(absl::StrSplit(actual, ' '));
  if (std::abs(expected_score - actual_score) > kEpsilon) {
    LOG(WARNING) << "Two sentence piece sequences are not equivalent! Left: " << expected
                 << ", Score: " << expected_score << ". Right: " << actual
                 << ", Score: " << actual_score << ".";
    return false;
  }
  return true;
...
EncodeResult Model::EncodeOptimized(absl::string_view normalized) const {
  ...
  struct BestPathNode {
    ...
    float best_path_score = 0;  // The total score of the best path ending at this node.
    int starts_at = -1;         // The starting position (in utf-8) of this node. The entire best
  ...
  while (starts_at < size) {
    ...
    bool has_single_node = false;
    const int mblen =
        std::min<int>(string_util::OneCharLen(normalized.data() + starts_at), size - starts_at);
    while (key_pos < size) {
      ...
      if (ret >= 0) {
        ...
        // User defined symbol receives extra bonus to always be selected.
        const auto score =
            IsUserDefinedInlined(ret) ? (length * max_score_ - 0.1) : GetScoreInlined(ret);
