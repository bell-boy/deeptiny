...
#define MSGPACK_DEFINE_MAP_IMPL(...)                                               \
  MSGPACK_PP_SEQ_TO_TUPLE(MSGPACK_PP_SEQ_FOR_EACH(MSGPACK_DEFINE_MAP_EACH_PROC, 0, \
...
#define MSGPACK_DEFINE_MAP(...)                                                                    \
  ...
  void msgpack_object(MSGPACK_OBJECT* msgpack_o, msgpack::zone& msgpack_z) const {                 \
    msgpack::type::make_define_map MSGPACK_DEFINE_MAP_IMPL(__VA_ARGS__)                            \
...
#define MSGPACK_DEFINE_MAP_IMPL(...)                                           \
  BOOST_PP_SEQ_TO_TUPLE(BOOST_PP_SEQ_FOR_EACH(MSGPACK_DEFINE_MAP_EACH_PROC, 0, \
  ...
  MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {                                        \
    ...
    struct pack<enum_name> {                                                                     \
      ...
      msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& msgpack_o,                    \
                                          const enum_name& msgpack_v) const {                    \
        return msgpack::operator<<(                                                              \
            msgpack_o, static_cast<msgpack::underlying_type<enum_name>::type>(msgpack_v));       \
      }                                                                                          \
    };                                                                                           \
    }                                                                                            \
    /** @cond */                                                                                 \
