...
MSGPACK_API_VERSION_NAMESPACE(v1) {
  ...
  class object_parser {
   private:
    ...
    struct elem {
      elem(msgpack::object const* p, std::size_t r) : rest(r), is_map(false), is_key(false) {
        as.obj_ptr = p;
      }

      elem(msgpack::object_kv const* p, std::size_t r) : rest(r), is_map(true), is_key(true) {
        as.kv_ptr = p;
      }
  ...
  struct object_pack_visitor {
    explicit object_pack_visitor(msgpack::packer<Stream>& pk) : m_packer(pk) {}
    bool visit_nil() {
      m_packer.pack_nil();
      return true;
    }
  ...
  struct object_stringize_visitor {
    explicit object_stringize_visitor(std::ostream& os) : m_os(os) {}
    bool visit_nil() {
      m_os << "null";
      return true;
    }
  ...
  struct aligned_zone_size_visitor {
    explicit aligned_zone_size_visitor(std::size_t& s) : m_size(s) {}
    ...
    bool start_array(uint32_t num_elements) {
      m_size += msgpack::aligned_size(sizeof(msgpack::object) * num_elements,
  ...
  template <typename T>
  inline object::implicit_type::operator T() {
    return obj.as<T>();
  }
  ...
  struct object_with_zone<msgpack::object> {
   ...
   private:
    struct object_with_zone_visitor {
     ...
     private:
      struct elem {
        elem(msgpack::object* obj) : is_obj(true) { as.obj = obj; }
        elem(msgpack::object_kv* kv) : is_obj(false) { as.kv = kv; }
  ...
  struct object_equal_visitor {
   ...
   private:
    struct elem {
      elem(msgpack::object const* obj) : is_obj(true) { as.obj = obj; }
      elem(msgpack::object_kv const* kv) : is_obj(false) { as.kv = kv; }
  ...
  template <typename T>
  inline bool operator==(const msgpack::object& x, const T& y) try {
  ...
  template <typename T>
  inline typename msgpack::enable_if<!msgpack::is_array<T>::value && !msgpack::is_pointer<T>::value,
                                     T&>::type
