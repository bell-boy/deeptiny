...
MSGPACK_API_VERSION_NAMESPACE(v1) {
  ...
  namespace detail {
  template <typename Target, typename Source, bool target_is_signed = std::is_signed<Target>::value,
            bool source_is_signed = std::is_signed<Source>::value,
            typename = typename std::enable_if<std::is_integral<Target>::value &&
                                               std::is_integral<Source>::value>::type>
  ...
  struct pack<std::chrono::time_point<Clock, Duration>> {
    ...
    msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o,
      ...
      int64_t nano_num =
          Duration::period::ratio::num * (1000000000L / Duration::period::ratio::den);
  ...
  struct object_with_zone<std::chrono::time_point<Clock, Duration>> {
    void operator()(msgpack::object::with_zone& o,
                    const std::chrono::time_point<Clock, Duration>& v) const {
      int64_t count = detail::integral_cast<int64_t>(v.time_since_epoch().count());
