...
MSGPACK_API_VERSION_NAMESPACE(v1) {
  ...
  inline auto tuple_cat(Tuples&&... args) -> decltype(std::tuple_cat(
  ...
  struct MsgpackTupleConverter {
    static void convert(msgpack::object const& o, Tuple& v) {
      ...
      if (o.via.array.size >= N)
        o.via.array.ptr[N - 1]
            .convert<typename std::remove_reference<decltype(v.template get<N - 1>())>::type>(
                v.template get<N - 1>());
    }
  };
  ...
  struct MsgpackTupleConverter<Tuple, 1> {
    static void convert(msgpack::object const& o, Tuple& v) {
      o.via.array.ptr[0]
          .convert<typename std::remove_reference<decltype(v.template get<0>())>::type>(
              v.template get<0>());
    }
  };
