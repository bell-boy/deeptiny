...
BOOST_AUTO_TEST_CASE(unpack_array_over_cpp11_off_ref) {
  ...
  try {
    ...
    std::size_t off = 0;
    msgpack::object_handle unp =
        msgpack::unpack(ss.str().c_str(), ss.str().size(), off, ref, MSGPACK_NULLPTR,
                        MSGPACK_NULLPTR, msgpack::unpack_limit(2, 0, 0, 0, 0));
    BOOST_CHECK(false);
  } catch (msgpack::array_size_overflow const&) {
    BOOST_CHECK(true);
  } catch (...) {
...
BOOST_AUTO_TEST_CASE(unpack_array_over_off_ref) {
  ...
  try {
    ...
    std::size_t off = 0;
    msgpack::object_handle unp;
    msgpack::unpack(unp, ss.str().c_str(), ss.str().size(), off, ref, MSGPACK_NULLPTR,
                    MSGPACK_NULLPTR, msgpack::unpack_limit(2, 0, 0, 0, 0));
    BOOST_CHECK(false);
  } catch (msgpack::array_size_overflow const&) {
    BOOST_CHECK(true);
