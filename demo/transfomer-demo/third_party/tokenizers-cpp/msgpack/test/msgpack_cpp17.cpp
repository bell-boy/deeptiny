...
BOOST_AUTO_TEST_CASE(vector_byte_pack_convert) {
  std::stringstream ss;
  std::vector<std::byte> val1{std::byte{0x01}, std::byte{0x02}, std::byte{0x7f}, std::byte{0x80},
                              std::byte{0xff}};

  msgpack::pack(ss, val1);
  std::string const& str = ss.str();

  char packed[] = {char(0xc4), char(0x05), char(0x01), char(0x02),
                   char(0x7f), char(0x80), char(0xff)};
...
BOOST_AUTO_TEST_CASE(array_byte_pack_convert) {
  std::stringstream ss;
  std::array<std::byte, 5> val1{std::byte{0x01}, std::byte{0x02}, std::byte{0x7f}, std::byte{0x80},
                                std::byte{0xff}};
...
BOOST_AUTO_TEST_CASE(carray_byte_pack_convert) {
  std::stringstream ss;
  std::byte val1[] = {std::byte{0x01}, std::byte{0x02}, std::byte{0x7f}, std::byte{0x80},
                      std::byte{0xff}};
...
BOOST_AUTO_TEST_CASE(variant_pack_unpack_as) {
...

  {
    ...
    msgpack::object_handle same_oh = msgpack::unpack(same_str.data(), same_str.size());
