...
BOOST_AUTO_TEST_CASE(msgpack_tuple) {
  ...
  msgpack::type::size_equal_only<msgpack::type::tuple<int, bool, std::string, int> > ret_seo4(
      ret_buf4);
  try {
    oh.get().convert(ret_seo4);
    BOOST_CHECK(false);
  ...
  msgpack::type::size_equal_only<msgpack::type::tuple<int, bool, std::string, int> > ret_seo6(
      ret_buf6);
  try {
