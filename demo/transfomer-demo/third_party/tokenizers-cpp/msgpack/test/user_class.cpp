...
class TestEnumMemberClass {
 public:
  TestEnumMemberClass() : t1(STATE_A), t2(STATE_B), t3(STATE_C) {}
...
class TestClass2 {
 public:
  TestClass2() : i(0), s("kzk") {
    for (unsigned int i = 0; i < kElements; i++) v.push_back(rand());
  }
  int i;
  string s;
  vector<int> v;
...
struct v_da_bottom : v_da_mid1, v_da_mid2 {
  int b;
  MSGPACK_DEFINE_ARRAY(MSGPACK_BASE_ARRAY(v_da_mid1), MSGPACK_BASE_ARRAY(v_da_mid2),
                       MSGPACK_BASE_ARRAY(v_da_top), b);
};
...
struct v_dm_bottom : v_dm_mid1, v_dm_mid2 {
  int b;
  MSGPACK_DEFINE_MAP(MSGPACK_BASE_MAP(v_dm_mid1), MSGPACK_BASE_MAP(v_dm_mid2),
                     MSGPACK_BASE_MAP(v_dm_top), b);
};
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
  ...
  struct object_with_zone<test_non_intrusive> {
    void operator()(msgpack::object::with_zone& o, const test_non_intrusive& t) const {
      o << t.name();
    }
  };
...
struct nvp_derived : nvp_base {
  int c;
  std::string d;
  MSGPACK_DEFINE_MAP(MSGPACK_NVP("ccc", c), MSGPACK_NVP("base", MSGPACK_BASE(nvp_base)),
                     MSGPACK_NVP("ddd", d));
};
...
BOOST_AUTO_TEST_CASE(nvp_combination) {
  ...
  BOOST_CHECK_EQUAL(
      std::string(obj.via.map.ptr[0].key.via.str.ptr, obj.via.map.ptr[0].key.via.str.size), "ccc");
  BOOST_CHECK_EQUAL(obj.via.map.ptr[0].val.via.i64, 3);

  BOOST_CHECK_EQUAL(
      std::string(obj.via.map.ptr[1].key.via.str.ptr, obj.via.map.ptr[1].key.via.str.size), "base");
  ...
  BOOST_CHECK_EQUAL(
      std::string(obj.via.map.ptr[2].key.via.str.ptr, obj.via.map.ptr[2].key.via.str.size), "ddd");
  BOOST_CHECK_EQUAL(
      std::string(obj.via.map.ptr[2].val.via.str.ptr, obj.via.map.ptr[2].val.via.str.size), "ABC");
