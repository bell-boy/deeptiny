...
BOOST_AUTO_TEST_CASE(simple_tuple_size_greater_than_as) {
  ...
  std::tuple<bool, std::string, double, int> val2 =
      oh.get().as<std::tuple<bool, std::string, double, int>>();
...
BOOST_AUTO_TEST_CASE(simple_array) {
  for (unsigned int k = 0; k < kLoop; k++) {
    ...
    for (unsigned int i = 0; i < kElements; i++) val1[i] = rand();
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_array_char) {
  for (unsigned int k = 0; k < kLoop; k++) {
    ...
    for (unsigned int i = 0; i < kElements; i++) val1[i] = static_cast<char>(rand());
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_array_unsigned_char) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    ...
    for (unsigned int i = 0; i < kElements; i++) val1[i] = static_cast<unsigned char>(rand());
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
    BOOST_CHECK_EQUAL(oh.get().type, msgpack::type::BIN);
...
BOOST_AUTO_TEST_CASE(simple_buffer_forward_list) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.push_front(rand());
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_unordered_map) {
  using type =
      unordered_map<int, int, test::hash<int>, test::equal_to<int>, test::map_allocator<int, int>>;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_unordered_multimap) {
  using type = unordered_multimap<int, int, test::hash<int>, test::equal_to<int>,
                                  test::map_allocator<int, int>>;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_unordered_multiset) {
  using type =
      unordered_multiset<int, test::hash<int>, test::equal_to<int>, test::set_allocator<int>>;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.insert(rand());
    msgpack::sbuffer sbuf;
...
inline bool operator==(no_def_con const& lhs, no_def_con const& rhs) { return lhs.i == rhs.i; }

inline bool operator!=(no_def_con const& lhs, no_def_con const& rhs) { return !(lhs == rhs); }
...
BOOST_AUTO_TEST_CASE(system_clock_64_max) {
  ...
  msgpack::pack(sbuf, val1);
  char packed_nano[] = {
      static_cast<char>(0xd7u), static_cast<char>(-1),
...
BOOST_AUTO_TEST_CASE(high_resolution_clock) {
  ...
  msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
  std::chrono::high_resolution_clock::time_point val2 =
      oh.get().as<std::chrono::high_resolution_clock::time_point>();
  BOOST_CHECK(val1 == val2);
...
BOOST_AUTO_TEST_CASE(timespec_pack_convert_32bit_sec) {
  std::stringstream ss;
  timespec val1{std::numeric_limits<decltype(std::declval<timespec>().tv_sec)>::is_signed
                    ? time_t(INT32_MAX)
                    : time_t(UINT32_MAX),
                0};
