...
BOOST_AUTO_TEST_CASE(simple_buffer_wstring) {
  for (unsigned int k = 0; k < kLoop; k++) {
    wstring val1;
    for (unsigned int i = 0; i < kElements; i++) val1 += L'a' + rand() % 26;
    msgpack::sbuffer sbuf;
...
BOOST_AUTO_TEST_CASE(simple_buffer_vector) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.push_back(rand());
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
    BOOST_CHECK_EQUAL(oh.get().type, msgpack::type::ARRAY);
...
BOOST_AUTO_TEST_CASE(simple_buffer_vector_char) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.push_back(static_cast<char>(rand()));
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_vector_unsigned_char) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.push_back(static_cast<unsigned char>(rand()));
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
    BOOST_CHECK_EQUAL(oh.get().type, msgpack::type::BIN);
...
BOOST_AUTO_TEST_CASE(simple_buffer_vector_uint8_t) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.push_back(static_cast<uint8_t>(rand()));
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
    BOOST_CHECK_EQUAL(oh.get().type, msgpack::type::BIN);
...
BOOST_AUTO_TEST_CASE(simple_buffer_vector_bool) {
  ...
  type val1;
  for (unsigned int i = 0; i < kElements; i++) val1.push_back(i % 2 ? false : true);
  msgpack::sbuffer sbuf;
  msgpack::pack(sbuf, val1);
  msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
  BOOST_CHECK_EQUAL(oh.get().type, msgpack::type::ARRAY);
...
BOOST_AUTO_TEST_CASE(simple_buffer_assoc_vector) {
  typedef msgpack::type::assoc_vector<int, int, test::less<int>,
                                      test::allocator<std::pair<int, int> > >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_map) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1[rand()] = rand();
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
    type const& val2 = oh.get().as<type>();
...
BOOST_AUTO_TEST_CASE(simple_buffer_set) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.insert(rand());
    msgpack::sbuffer sbuf;
    msgpack::pack(sbuf, val1);
    msgpack::object_handle oh = msgpack::unpack(sbuf.data(), sbuf.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_multimap) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    ...
    for (it = val1.begin(); it != val1.end(); ++it) v1.push_back(make_pair(it->first, it->second));
    for (it = val2.begin(); it != val2.end(); ++it) v2.push_back(make_pair(it->first, it->second));
    BOOST_CHECK_EQUAL(val1.size(), val2.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_multiset) {
  ...
  for (unsigned int k = 0; k < kLoop; k++) {
    ...
    vector<int> v1, v2;
    type::const_iterator it;
    for (it = val1.begin(); it != val1.end(); ++it) v1.push_back(*it);
    for (it = val2.begin(); it != val2.end(); ++it) v2.push_back(*it);
    BOOST_CHECK_EQUAL(val1.size(), val2.size());
    BOOST_CHECK_EQUAL(v1.size(), v2.size());
    sort(v1.begin(), v1.end());
    sort(v2.begin(), v2.end());
...
BOOST_AUTO_TEST_CASE(simple_buffer_tr1_unordered_map) {
  typedef tr1::unordered_map<int, int, test::tr1_hash<int>, test::equal_to<int>,
...
BOOST_AUTO_TEST_CASE(simple_buffer_tr1_unordered_multimap) {
  typedef tr1::unordered_multimap<int, int, test::tr1_hash<int>, test::equal_to<int>,
                                  test::allocator<std::pair<const int, int> > >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_tr1_unordered_set) {
  typedef tr1::unordered_set<int, test::tr1_hash<int>, test::equal_to<int>, test::allocator<int> >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
    for (unsigned int i = 0; i < kElements; i++) val1.insert(rand());
    ...
    BOOST_CHECK_EQUAL(val1.size(), val2.size());
    type::const_iterator it;
    for (it = val1.begin(); it != val1.end(); ++it) BOOST_CHECK(val2.find(*it) != val2.end());
  }
}
...
BOOST_AUTO_TEST_CASE(simple_buffer_tr1_unordered_multiset) {
  typedef tr1::unordered_multiset<int, test::tr1_hash<int>, test::equal_to<int>,
                                  test::allocator<int> >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_unordered_map) {
  typedef unordered_map<int, int, test::hash<int>, test::equal_to<int>,
                        test::allocator<std::pair<const int, int> > >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
...
BOOST_AUTO_TEST_CASE(simple_buffer_unordered_multimap) {
  typedef unordered_multimap<int, int, test::hash<int>, test::equal_to<int>,
                             test::allocator<std::pair<const int, int> > >
      type;
  for (unsigned int k = 0; k < kLoop; k++) {
    type val1;
